...
url = "https://amjskrqaoiuabscecmji.supabase.co"   #supabase
...
def obtener_sol(anio, mes, dia, hora, minuto, lat, lon):
    print(f"🛰️ Procesando Sol → {anio}-{mes}-{dia} {hora}:{minuto}, lat: {lat}, lon: {lon}")
...
def obtener_luna(anio, mes, dia, hora, minuto, lat, lon):
    ...
    print(f"🛰️ Recibido en /luna → {anio}-{mes}-{dia} {hora}:{minuto}, lat: {lat}, lon: {lon}")
    offset = obtener_offset_horario(lat, lon)
    hora_utc_decimal = hora + minuto / 60 - offset
    jd = swe.julday(anio, mes, dia, hora_utc_decimal)
    ...
    print(f"JD: {jd}")
    print(f"Lat: {lat}, Lon: {lon}, Offset: {offset}, Hora local: {hora}:{minuto}, Hora UTC: {hora_utc_decimal}")
...
def obtener_elemento(planetas_en_signos):
    ...
    elementos_dominantes = [elemento for elemento, cantidad in contador_elementos.items() if cantidad == max_valor]
...
def obtener_polaridad(planetas_en_signos):
    ...
    polaridad_por_signo = {
        "Aries": "Positivo", "Leo": "Positivo", "Sagitario": "Positivo",
        "Géminis": "Positivo", "Libra": "Positivo", "Acuario": "Positivo",
        "Tauro": "Negativo", "Cáncer": "Negativo", "Virgo": "Negativo",
        "Escorpio": "Negativo", "Capricornio": "Negativo", "Piscis": "Negativo"
    }
...
def obtener_modalidad(planetas_en_signos):
    ...
    modalidad_por_signo = {
        "Aries": "Cardinal", "Leo": "Fija", "Sagitario": "Mutable",
        "Cáncer": "Cardinal", "Escorpio": "Fija", "Piscis": "Mutable",
        "Géminis": "Mutable", "Libra": "Cardinal", "Acuario": "Fija",
        "Tauro": "Fija", "Virgo": "Mutable", "Capricornio": "Cardinal"
    }
    ...
    modalidad_dominantes = [m for m, cantidad in contador_modalidad.items() if cantidad == max_valor]
...
def procesar_y_guardar(anio, mes, dia, hora, minuto, lat, lon):
...
registro = {
    ...
    "gr_sol": sol["grados_en_signo"],
    "c_sol": sol["casa"],
    "ascen": obtener_ascendente(anio, mes, dia, hora, minuto, lat, lon),
    "gr_asc": obtener_ascendente(anio, mes, dia, hora, minuto, lat, lon)["grados_en_signo"],
    "gr_luna": luna["grados_en_signo"],
    "c_luna": luna["casa"],
    "gr_merc": mercurio["grados_en_signo"],
    "c_merc": mercurio["casa"],
    "gr_venus": venus["grados_en_signo"],
    "c_venus": venus["casa"],
    "gr_marte": marte["grados_en_signo"],
    "c_marte": marte["casa"],
    "gr_jupiter": jupiter["grados_en_signo"],
    "c_jupiter": jupiter["casa"],
    "gr_satur": saturno["grados_en_signo"],
    "c_satur": saturno["casa"],
    "gr_urano": urano["grados_en_signo"],
    "c_urano": urano["casa"],
    "gr_neptu": neptuno["grados_en_signo"],
    "c_neptu": neptuno["casa"],
    "gr_pluto": pluton["grados_en_signo"],
    "c_pluto": pluton["casa"],
    "nodoN": obtener_nodoN(anio, mes, dia, hora, minuto, lat, lon),
    "gr_nodoN": obtener_nodoN(anio, mes, dia, hora, minuto, lat, lon)["grados_en_signo"],
    "c_nodoN": obtener_nodoN(anio, mes, dia, hora, minuto, lat, lon)["casa"],
    "nodoS": obtener_nodo_sur(anio, mes, dia, hora, minuto, lat, lon),
    "gr_nodoS": obtener_nodo_sur(anio, mes, dia, hora, minuto, lat, lon)["grados_en_signo"],
    "c_nodoS": obtener_nodo_sur(anio, mes, dia, hora, minuto, lat, lon)["casa"],
    "gr_lilith": lilith["grados_en_signo"],
    "c_lilith": lilith["casa"],
    "gr_quiron": quiron["grados_en_signo"],
    "c_quiron": quiron["casa"]
}

data, count = supabase.table("datoscc").insert(registro).execute()

if data:
    print("✅ Registro insertado correctamente.")
else:
    print("❌ Error al insertar:", count)
...
if __name__ == '__main__':
    procesar_y_guardar()  # la función principal que coordina todo
